...
int32_t scap_linux_create_iflist(struct scap_platform* platform)
{
	...
	//
	if(handle->m_storage.m_addrlist != NULL)
	{
		scap_free_iflist(handle->m_storage.m_addrlist);
		handle->m_storage.m_addrlist = NULL;
	}
	...
	//
	handle->m_storage.m_addrlist = (scap_addrlist*)malloc(sizeof(scap_addrlist));
	if(!handle->m_storage.m_addrlist)
	{
		snprintf(handle->m_lasterr,	SCAP_LASTERR_SIZE, "getifaddrs allocation failed(1)");
		return SCAP_FAILURE;
	}
	addrlist = handle->m_storage.m_addrlist;
