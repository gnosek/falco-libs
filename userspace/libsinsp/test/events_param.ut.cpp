...
TEST_F(sinsp_with_test_input, charbuf_empty_param)
{
	...
	ASSERT_STREQ(evt->get_param<std::string_view>(1).data(), "<NA>");
	...
	ASSERT_STREQ(evt->get_param<std::string_view>(0).data(), "<NA>");
...
TEST_F(sinsp_with_test_input, param_charbuf_len_1)
{
	...
	ASSERT_STREQ(evt->get_param<std::string_view>(1).data(), "");
}
