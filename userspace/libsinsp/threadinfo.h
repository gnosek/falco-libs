...
public:
	...
	*/
	...
	std::atomic<uint32_t> m_uid;            ///< uid
	std::atomic<uint32_t> m_gid;            ///< gid
	std::atomic<uint32_t> m_loginuid;       ///< loginuid
	...
	std::atomic<uint64_t> m_exe_ino;        ///< executable inode ino
	std::atomic<uint64_t> m_exe_ino_ctime;  ///< executable inode ctime (last status change time)
	std::atomic<uint64_t> m_exe_ino_mtime;  ///< executable inode mtime (last modification time)
