...
uint8_t* sinsp_filter_check_thread::extract_single(sinsp_evt* evt,
	...
	*len = 0;
	...
	case TYPE_PID:
		...
		RETURN_EXTRACT_VAR(m_val.s64);
	case TYPE_SID:
		m_val.s64 = tinfo->m_sid.load();
		RETURN_EXTRACT_VAR(m_val.s64);
	case TYPE_VPGID:
		m_val.s64 = tinfo->m_vpgid.load();
		RETURN_EXTRACT_VAR(m_val.s64);
	case TYPE_SID_NAME:
		m_tstr = m_inspector->m_thread_manager->get_ancestor_field_as_string(
		        tinfo,
		        [](sinsp_threadinfo* t) { return t->m_sid.load(); },
	...
	case TYPE_VPGID_NAME:
		m_tstr = m_inspector->m_thread_manager->get_ancestor_field_as_string(
		        tinfo,
		        [](sinsp_threadinfo* t) { return t->m_vpgid.load(); },
		        [](sinsp_threadinfo* t) { return t->get_comm(); },
		        true);
		RETURN_EXTRACT_STRING(m_tstr);
	case TYPE_VPGID_EXE:
	...
	case TYPE_FD_STDERR_NAME: {
		...
		m_tstr = *fdinfo->m_name.lock();
		RETURN_EXTRACT_STRING(m_tstr);
	}
	case TYPE_PGID:
		m_val.s64 = tinfo->m_pgid.load();
		RETURN_EXTRACT_VAR(m_val.s64);
	case TYPE_PGID_NAME:
		m_tstr = m_inspector->m_thread_manager->get_ancestor_field_as_string(
		        tinfo,
		        [](sinsp_threadinfo* t) { return t->m_pgid.load(); },
		        [](sinsp_threadinfo* t) { return t->get_comm(); });
		RETURN_EXTRACT_STRING(m_tstr);
	case TYPE_PGID_EXE:
		m_tstr = m_inspector->m_thread_manager->get_ancestor_field_as_string(
...
bool sinsp_filter_check_thread::compare_full_aexepath(sinsp_evt* evt) {
	...
	bool found = false;
	sinsp_thread_manager::visitor_func_t visitor = [this, &found](sinsp_threadinfo* pt) {
		if(compare_rhs(m_cmpop, PT_CHARBUF, pt->m_exepath.lock()->c_str()) == true) {
			found = true;
