...
void sinsp_parser::parse_clone_exit_caller(sinsp_evt &evt,
	...
	*child_tinfo->m_exe.lock() = evt.get_param(1)->as<std::string>();
	...
	if(valid_caller) {
		...
		child_tinfo->m_exe_lower_layer.store(caller_tinfo->m_exe_lower_layer.load());
		...
		*child_tinfo->m_root.lock() = *caller_tinfo->m_root.lock();
		...
		child_tinfo->m_vpgid.store(caller_tinfo->m_vpgid.load());
...
void sinsp_parser::parse_clone_exit_child(sinsp_evt &evt, sinsp_parser_verdict &verdict) const {
	...
	*child_tinfo->m_exe.lock() = evt.get_param(1)->as<std::string>();
	...
	if(valid_lookup_thread) {
		...
		child_tinfo->m_exe_lower_layer.store(lookup_tinfo->m_exe_lower_layer.load());
		...
		*child_tinfo->m_root.lock() = *lookup_tinfo->m_root.lock();
		...
		child_tinfo->m_vpgid.store(lookup_tinfo->m_vpgid.load());
